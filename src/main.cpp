...
void loadData(std::vector<Subject> &subjects)
{
  ...
  if (!iFile.is_open())
  {
    std::cout << "Error opening file!No data file found ! Starting fresh." << std::endl;
    return;
  }
  int subCount;
  ...
  if (!iFile >> subCount || subCount < 0)
  {
    std::cout << "No subject count found .! Starting fresh.." << std::endl;
    return;
  };
  iFile.ignore();

  std::vector<Subject> tempSubjects; // temporary subject storage

  for (int i = 0; i < subCount; i++)
  {
    Subject s; // construct a fresh subject
    if (!std::getline(iFile, s.name))
    {
      std::cout << "Corrupted subject name! Starting Fresh..." << std::endl;
      return;
    };

    int assCount;

    if (!iFile >> assCount || assCount < 0)
    {
      std::cout << "Corrupted assingment count! Starting Fresh..." << std::endl;
      return;
    };
    iFile.ignore();
    ...
    {
      Assignment a;
      if(!std::getline(iFile, a.title, '|') || 
      !std::getline(iFile, a.description, '|') ||
      !std::getline(iFile, a.deadline, '|'))
      {
        std::cout << "Corrupted assignment data!Starting Fresh..." << std::endl;
        return;
      }
      ...
      if(!(iFile >> status))
      {
        std::cout << "Corrupted assignment status!Starting Fresh..." << std::endl;
        return;
      };
      iFile.ignore();
      a.status = status;
...
void viewAssignments(std::vector<Subject> &subjects)
{
  ...
  if (subjectIndex == -1)
    return;
  if (subjects[subjectIndex].assignments.size() <= 0)
    std::cout << "No assignments!";
  else
  {
    // print assignments
    auto &assignments = subjects[subjectIndex].assignments;
    ...
    for (std::size_t i = 0; i < subjects[subjectIndex].assignments.size(); i++)
    {
      ...
      if (subjects[subjectIndex].assignments[i].status)
        std::cout << "[✔]\t" << std::endl;
      else
        std::cout << "[ ]\t" << std::endl;
    }
